...
export interface FirestoreCaseDoc extends CaseSettingsSnapshot {
    ...
    /** KB metadata for reproducibility (pinned release) */
    kbMeta?: {
        releaseId?: string;
        medical?: { version: string; hash: string; source: 'json' | 'firestore' | 'firestore_fallback' | 'forced' };
        treatments?: Record<string, { version: string; hash: string; source: 'json' | 'firestore' | 'firestore_fallback' | 'forced' }>;
        combinability?: { version: string; hash: string; source: 'json' | 'firestore' | 'firestore_fallback' | 'forced' };
    };
}
