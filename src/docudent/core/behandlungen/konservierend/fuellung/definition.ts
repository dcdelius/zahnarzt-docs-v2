 ...
 * 
 * REFACTORED: Lädt jetzt aus treatments/fuellung/unified.json (SSOT)
 * Keine hardcodierten Chip-Definitionen mehr!
 * 
 * Single Source of Truth: treatments/fuellung/unified.json
 */
...
export const FILLING_TREATMENT: TreatmentDefinition = {
    ...
    // Texte (nicht in unified.json vorhanden, hier hardcoded als Fallback)
    consentText: '',
    consentTexts: { kurz: '', mittel: '', lang: '' } as TextSnippets,
    dismissalText: '',
    dismissalTexts: { kurz: '', mittel: '', lang: '' } as TextSnippets,
    ...
    // Billing & Upsell rules (leer - werden aus unified.json gelesen)
    billingRules: [],
    upsells: [],
