...
export function processChipsToBilling(
    ...
    const orderedChipIds = [...activeChipIds].sort((a, b) => {
        ...
        const idxA = chipA ? treatment.phasen_reihenfolge.indexOf(chipA.phase) : -1;
        const idxB = chipB ? treatment.phasen_reihenfolge.indexOf(chipB.phase) : -1;
        const phaseA = idxA === -1 ? 999 : idxA;
        const phaseB = idxB === -1 ? 999 : idxB;
    ...
    for (const chipId of orderedChipIds) {
        ...
        const textSnippets = chip.textSnippets ?? {};
        let text = textSnippets[textLength] || textSnippets.mittel || '';
        if (chip.variablen) {
...
export function loadRules(treatmentId: string): RuleDefinition[] {
    ...
    switch (normalized) {
        ...
        default:
            if (normalized) {
                console.warn(`[TreatmentEngine] No rules found for: ${normalized}`);
            }
    }
