...
export const FUELLUNG_AUFKLAERUNG_CLAUSES: AufklaerungClause[] = [
    ...
    },
    {
        ...
        when: {
            anyChips: [CANONICAL_CHIP_IDS.CP, CANONICAL_CHIP_IDS.P],
        },
        text: {
            kurz: 'CP: Endo-Risiko besprochen.',
            mittel: 'Bei der Überkappung wurde über das Risiko eines späteren Vitalitätsverlusts aufgeklärt.',
            lang: 'Eine Überkappung wurde durchgeführt. Der Patient wurde über das Risiko eines späteren Vitalitätsverlusts der Pulpa und die daraus möglicherweise resultierende Notwendigkeit einer Wurzelkanalbehandlung aufgeklärt.',
        },
        priority: 21,
    },
...
export function evaluateAufklaerungClauses(
    ...
    return clauses
        .filter(clause => {
            ...
            // Check requiresMkV (only enforce when context explicitly provides MKV flag)
            if (typeof when.requiresMkV === 'boolean' && typeof context.hasMKV === 'boolean') {
                if (context.hasMKV !== when.requiresMkV) return false;
            }
